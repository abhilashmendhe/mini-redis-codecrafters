...
pub async fn fetch_list(listkey: &String, kv_map: SharedMapT) -> VecDeque<String> {
    let kv_map_gaurd = kv_map.lock().await;
    let list_items = match kv_map_gaurd.get(listkey) {
        Some(value_struct) => match &value_struct.value {
...
pub async fn compute_index(index: &String, list_len: i64) -> Result<i64, RedisErrors> {
