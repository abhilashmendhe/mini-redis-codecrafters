        ...
        if let Some(vs) = kv_gaurd.get_mut(key) {
            ...
            let res_form = match vs.mut_value() {
                crate::basics::kv_ds::Value::SORTED_SET(sorted_set) => {
                    insert_values_in_sorted_set(&set_values, sorted_set)?
                }
            ...
            let res = insert_values_in_sorted_set(&set_values, &mut sorted_set)?;

            let vs = ValueStruct::new(
                crate::basics::kv_ds::Value::SORTED_SET(sorted_set),
                None,
                None,
            );
...
fn insert_values_in_sorted_set(
    ...
    while ind < set_values.len() {
        ...
        let sorted_values = SortedSetValues {
            v1,
            v2: v2.to_string(),
        };
