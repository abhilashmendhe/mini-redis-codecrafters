...
use crate::{
    ...
    streams::{
        ...
        stream_utils::{
            extract_stream_data, extract_stream_id, extract_stream_read_data,
            extract_stream_read_data_block,
        },
    },
};

pub async fn type_ops(key: String, kv_map: SharedMapT) -> Result<String, RedisErrors> {
    let value_type_form = {
                ...
                for ss in stream_list {
                    if s_epoch <= ss.epoch && e_epoch >= ss.epoch {
                        if s_seq_num == -1 && e_seq_num == -1 {
                            ...
                            if s_seq_num as usize <= ss.seq_number
                                && e_seq_num as usize >= ss.seq_number
                            {
...
pub async fn xread(streams_ids: &Vec<String>, kv_map: SharedMapT) -> Result<String, RedisErrors> {
    // println!("{:?}",streams_ids);
    if streams_ids[0].eq("block") {
        ...
        let stream_key_last_value: Arc<Mutex<HashMap<String, (u128, isize)>>> =
            Arc::new(Mutex::new(HashMap::new()));
        tokio::spawn(async move {
            println!("Started to fetch....");
            let mut t = 0_u64;
            while let Ok(data_form) = extract_stream_read_data_block(
                block_streams_ids.clone(),
                kv_map.clone(),
                stream_key_last_value.clone(),
            )
            .await
            {
