                ...
                while let Some((cmds, _end)) = try_parse_resp(&buffer).await {
                    ...
                    if cmds[0].eq("AUTH") {
                        let form = if cmds.len() < 2 {
                            ...
                            let user = &cmds[1];
                            let password = &cmds[2];
                            auth(user, password, acl_t.clone(), connections1.clone(), sock_addr).await 
                        };
