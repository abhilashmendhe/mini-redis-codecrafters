...
pub async fn run_redis_node() -> Result<(), RedisErrors> {
    ...
    // Spawn redis server to listen from redis-cli clients
    let connections1 = Arc::clone(&connections);
        ...
        loop {
            {
                ...
                if role_guard.role() != "slave" {
                    println!("Not a slave. Stop!!");
                    break;
                }
            }

            println!("Handshake with redis(master) initiated....");
            ...
            println!("Retrying handshake in 5 seconds...");
            tokio::time::sleep(tokio::time::Duration::from_secs(5)).await;
        }
    });

    let replica_info3 = Arc::clone(&replica_info);
    {
        let role_guard = replica_info3.lock().await;
        if role_guard.role() != "slave" {
            println!("Master(Redis) node is running!!!");
        } else {
            println!("Slave(Redis) node is running and connected with master!!!");
        }
    }
